...
export async function POST(req, res) {
  try {
    ...
    await prisma.$executeRaw`TRUNCATE TABLE vegetation RESTART IDENTITY CASCADE;`;
    ...
    for (const feature of geojson.features) {
      ...
      if (feature.geometry.type === 'Polygon' || feature.geometry.type === 'MultiPolygon') {
        ...
        // Optional: Grab the name if OSM provides it, otherwise call it 'Unnamed Vegetation'
        const name = feature.properties?.name || 'Unnamed Vegetation';

        try {
          await prisma.$executeRaw`
          INSERT INTO vegetation (name, geom) 
          VALUES (
            ${name}, 
    ...
    console.log(`✅ Successfully seeded ${successCount} real vegetation areas into PostGIS!`);
